...
public class BlockHTOven extends BlockContainerMF{
	...
	public BlockHTOven(boolean isActive, int tier) {
	...
	public Item getItemDropped(int i, Random random, int j) {
		return Item.getItemFromBlock(this.tier == 1 ? MFBlocks.HTOvenIdle : MFBlocks.MetalOvenIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if(!world.isRemote) {
			if(!player.isSneaking()){
				player.openGui(MFMod.INSTANCE, MFGuiHandler.OVEN, world, x, y, z);
			}
		}
		return true;
	}
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return (new TileEntityOven().setTier(this.tier));
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack) {
		...
		if(itemstack.hasDisplayName()) {
			((TileEntityOven)world.getTileEntity(x, y, z)).getInventoryName();
		}
	}
	...
	public static void updateMachineState(boolean active, World world, int xCoord, int yCoord, int zCoord) {
		...
		if(world.getBlock(xCoord, yCoord, zCoord) == MFBlocks.HTOvenIdle && active) {
			world.setBlock(xCoord, yCoord, zCoord, MFBlocks.HTOvenActive);
		}else if(world.getBlock(xCoord, yCoord, zCoord) == MFBlocks.HTOvenActive && !active){
			world.setBlock(xCoord, yCoord, zCoord, MFBlocks.HTOvenIdle);
		}
		
		if(world.getBlock(xCoord, yCoord, zCoord) == MFBlocks.MetalOvenIdle && active) {
			world.setBlock(xCoord, yCoord, zCoord, MFBlocks.MetalOvenActive);
		}else if(world.getBlock(xCoord, yCoord, zCoord) == MFBlocks.MetalOvenActive && !active){
			world.setBlock(xCoord, yCoord, zCoord, MFBlocks.MetalOvenIdle);
		}
