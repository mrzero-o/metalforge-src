...
public class EntityArrowSpecial extends Entity implements IProjectile
{
	private boolean isFreezeArrow = false;
	private boolean isBombArrow = false;
    ...
    public EntityArrowSpecial(World p_i1753_1_)
    {
        super(p_i1753_1_);
    ...
    public EntityArrowSpecial(World p_i1754_1_, double p_i1754_2_, double p_i1754_4_, double p_i1754_6_)
    {
        super(p_i1754_1_);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityArrowSpecial(World p_i1755_1_, EntityLivingBase p_i1755_2_, EntityLivingBase p_i1755_3_, float p_i1755_4_, float p_i1755_5_)
    {
        super(p_i1755_1_);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityArrowSpecial(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_)
    {
        super(p_i1756_1_);
        this.renderDistanceWeight = 10.0D;
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null)
                {
                    ...
                    Entity hit = movingobjectposition.entityHit;
                    ...
                    if (hit.attackEntityFrom(damagesource, (float)k))
                    {
                        if (hit instanceof EntityLivingBase)
                        {
                            ...
                            if(!(hit instanceof EntityEnderman)){
                            	this.setDead();
                            	this.doSpecialAction((EntityLivingBase)living);
                            }
                        }
    ...
    private EntityLivingBase freezemob(EntityLivingBase living, int multip, int timeInTicks){
    	living.addPotionEffect(new PotionEffect(Potion.moveSlowdown.getId(), multip, timeInTicks));
    	return living;
    }
    /*
     * Apart from here, you can customize everything the way you want
     */
    private void doSpecialAction(EntityLivingBase living){
    	if(!worldObj.isRemote){
        	if(this.isFreezeArrow && !this.isBombArrow){
        		living.addPotionEffect(new PotionEffect(Potion.moveSlowdown.getId(), 45, 2));
        	}if(this.isBombArrow && !this.isFreezeArrow){
            	boolean shouldBreakBlocks = false;
            	worldObj.createExplosion(living, posX, posY, posZ, 1.0F, shouldBreakBlocks);
        	}
    	}
    }
    
    public EntityArrowSpecial setIsFreezing(){
    	this.isFreezeArrow = true;
    	return this;
    }
    
    public EntityArrowSpecial setIsExplosive(){
    	this.isBombArrow = true;
    	return this;
    }
}